        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {navItems.map(({ path, label, icon }) => {
            ...
            return (
              ...
              >
                {isReplayNav && isRunning ? icons.spinner : icon}
                <span>{label}</span>
                {isReplayNav && isRunning && (
        ...
        {isRunning && (
          <div className="mx-3 mb-3 px-3 py-2 bg-amber-500/10 border border-amber-500/20 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-amber-400 text-xs font-medium">Replay in progress...</span>
            </div>
          </div>
        )}
